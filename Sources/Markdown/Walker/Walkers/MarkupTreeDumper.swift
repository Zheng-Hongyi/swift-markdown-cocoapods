...
extension String {
  ...
  fileprivate var newlineEscaped: String {
...
struct MarkupTreeDumper: MarkupWalker {
  ...
  private var lineIndentPrefix: String {
    ...
    for (depth, element) in path.enumerated().reversed() {
      guard let lastChildIndex = element.parent?.children.reversed().first?.indexInParent,
        lastChildIndex != element.indexInParent
      else {
        if depth > 0 {
          prefix.append("   ")
        }
  ...
  private mutating func indentLiteralBlock(
    _ string: String, from element: Markup, countLines: Bool = false
  ) -> String {
    path.append(element)
  ...
  mutating func visitBlockDirective(_ blockDirective: BlockDirective) {
    ...
    dump(
      blockDirective,
      customDescription: "name: \(blockDirective.name.debugDescription)\(argumentsDescription)")
  }
