...
extension TableCellContainer {
  ...
  public var cells: LazyMapSequence<MarkupChildren, Table.Cell> {
  ...
  public mutating func setCells<Cells: Sequence>(_ newCells: Cells)
  where Cells.Element == Table.Cell {
    replaceCellsInRange(0..<childCount, with: newCells)
  }

  /// Replace cells in a range with a sequence of cells.
  public mutating func replaceCellsInRange<Cells: Sequence>(
    _ range: Range<Int>, with incomingCells: Cells
  ) where Cells.Element == Table.Cell {
  ...
  public mutating func appendCell(_ cell: Table.Cell) {
