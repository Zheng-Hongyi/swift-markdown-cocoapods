...
public struct _MarkupData {
  ...
  func replacingSelf(_ newRaw: RawMarkup, preserveRange: Bool = false) -> _MarkupData {
    if let parent = parent {
      let newParent = parent._data.substitutingChild(
        newRaw, at: indexInParent, preserveRange: preserveRange)
      return newParent.child(at: indexInParent)!._data
    } else {
      return _MarkupData(
        AbsoluteRawMarkup(
          markup: newRaw, metadata: MarkupMetadata(id: .newRoot(), indexInParent: 0)), parent: nil)
    }
  }

  /// Returns a new `MarkupData` with the given child now at the `index`.
  func substitutingChild(_ rawChild: RawMarkup, at index: Int, preserveRange: Bool = false)
    -> Markup
  {
