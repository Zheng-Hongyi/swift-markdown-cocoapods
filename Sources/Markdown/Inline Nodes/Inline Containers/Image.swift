...
extension Image {
  ...
  public init<Children: Sequence>(source: String? = nil, title: String? = nil, _ children: Children)
  where Children.Element == RecurringInlineMarkup {
    ...
    try! self.init(
      .image(
        source: sourceToUse, title: titleToUse, parsedRange: nil, children.map { $0.raw.markup }))
  }

  /// Create an image from a source and zero or more child inline elements.
  public init(source: String? = nil, title: String? = nil, _ children: RecurringInlineMarkup...) {
    ...
    set {
      ...
      if let s = newValue, s.isEmpty {
        _data = _data.replacingSelf(
          .image(source: nil, title: title, parsedRange: nil, _data.raw.markup.copyChildren()))
      } else {
        _data = _data.replacingSelf(
          .image(source: newValue, title: title, parsedRange: nil, _data.raw.markup.copyChildren()))
      }
    }
  }
  ...
  public var title: String? {
    ...
    set {
      ...
      if let t = newValue, t.isEmpty {
        _data = _data.replacingSelf(
          .image(source: source, title: nil, parsedRange: nil, _data.raw.markup.copyChildren()))
      } else {
        _data = _data.replacingSelf(
          .image(source: source, title: newValue, parsedRange: nil, _data.raw.markup.copyChildren())
        )
      }
    }
  }
