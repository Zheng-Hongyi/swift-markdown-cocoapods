...
extension InlineAttributes {
  ...
  public init<Children: Sequence>(attributes: String, _ children: Children)
  where Children.Element == RecurringInlineMarkup {
    try! self.init(
      .inlineAttributes(attributes: attributes, parsedRange: nil, children.map { $0.raw.markup }))
  }

  /// Create a set of custom attributes applied to zero or more child inline elements.
  public init(attributes: String, _ children: RecurringInlineMarkup...) {
  ...
  public var attributes: String {
    ...
    set {
      _data = _data.replacingSelf(
        .inlineAttributes(attributes: newValue, parsedRange: nil, _data.raw.markup.copyChildren()))
    }
  }
