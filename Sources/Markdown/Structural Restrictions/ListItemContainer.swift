...
extension ListItemContainer {
  ...
  ///   must be a `ListItem`.
  public var listItems: LazyMapSequence<MarkupChildren, ListItem> {
    return children.lazy.map { $0 as! ListItem }
  }

  /// Replace all list items with a sequence of items.
  public mutating func setListItems<Items: Sequence>(_ newItems: Items)
  where Items.Element == ListItem {
    replaceItemsInRange(0..<childCount, with: newItems)
  }

  /// Replace list items in a range with a sequence of items.
  public mutating func replaceItemsInRange<Items: Sequence>(
  ...
  public mutating func appendItem(_ item: ListItem) {
