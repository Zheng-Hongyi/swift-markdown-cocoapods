...
final class RawMarkupTests: XCTestCase {
  func testHasSameStructureAs() {
    ...
    do {  // Empty match
      XCTAssert(
        RawMarkup.document(parsedRange: nil, []).hasSameStructure(
          as: .document(parsedRange: nil, [])))
    }
    ...
    do {  // Different child count
      let document1 = RawMarkup.document(parsedRange: nil, [.paragraph(parsedRange: nil, [])])
      let document2 = RawMarkup.document(
        parsedRange: nil, [.paragraph(parsedRange: nil, []), .thematicBreak(parsedRange: nil)])
      XCTAssertFalse(document1.hasSameStructure(as: document2))
    }

    do {  // Same child count, different structure, nested
      let document1 = RawMarkup.document(
        parsedRange: nil,
        [
          .paragraph(
            parsedRange: nil,
