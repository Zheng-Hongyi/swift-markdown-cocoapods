...
class ParsedRangePreservedAfterEditingTests: XCTestCase {
  ...
  func testParsed() {
    ...
    document.setBlockChildren(
      document.children.map { child -> BlockMarkup in
    ...
    XCTAssertEqual(
      SourceLocation(
        line: 1, column: 1, source: nil)..<SourceLocation(line: 1, column: 30, source: nil),
      document.child(at: 0)?.range)
    XCTAssertNil(document.child(at: 1)?.range)
    XCTAssertEqual(
      SourceLocation(
        line: 6, column: 1, source: nil)..<SourceLocation(line: 6, column: 30, source: nil),
