...
let everythingDocument = Document(
  parsing: try! String(
    contentsOf: Bundle.module.url(forResource: "Everything", withExtension: "md")!))
...
class MarkupRewriterTests: XCTestCase {
  ...
  func testNullRewriter() {
    ...
    try! markup.debugDescription().write(
      to: .init(fileURLWithPath: "/tmp/old.txt"), atomically: true, encoding: .utf8)
    try! shouldBeSame.debugDescription().write(
      to: .init(fileURLWithPath: "/tmp/new.txt"), atomically: true, encoding: .utf8)
