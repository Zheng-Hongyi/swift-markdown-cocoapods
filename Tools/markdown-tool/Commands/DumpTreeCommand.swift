...
extension MarkdownCommand {
  ...
  struct DumpTree: ParsableCommand {
    static let configuration = CommandConfiguration(
      ...
      abstract: "Dump the parsed standard input as a tree representation for debugging")

    @Argument(
      help: "Optional input file path of a Markdown file to format; default: standard input")
    var inputFilePath: String?

    @Flag(
      inversion: .prefixedNo, exclusivity: .chooseLast,
      help: "Print source locations for each element where applicable")
    var sourceLocations: Bool = false

    @Flag<Bool>(
      inversion: .prefixedNo, exclusivity: .chooseLast,
      help: "Print internal unique identifiers for each element")
    ...
    @Flag<Bool>(
      inversion: .prefixedNo, exclusivity: .chooseLast,
      help: "Parse a minimal set of Doxygen commands (requires --parse-block-directives)")
